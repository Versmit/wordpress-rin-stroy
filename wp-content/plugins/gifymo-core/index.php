 ...
 * used, this is required. Version: 1.0.8 Author: themelexus Author URI:  http://www.themelexus.com/ License: GNU/GPLv3
 * http://www.gnu.org/licenses/gpl-3.0.html
 */

final class GifymoCore {
    /**
     * @var GifymoCore
     */
    ...
    public function setup_constants() {
        if (!defined('GIFYMO_CORE_VERSION')) {
            define('GIFYMO_CORE_VERSION', '1.0.8');
        }

        if (!defined('GIFYMO_CORE_PLUGIN_DIR')) {
            define('GIFYMO_CORE_PLUGIN_DIR', plugin_dir_path(__FILE__));
        }

        if (!defined('GIFYMO_CORE_PLUGIN_URL')) {
            define('GIFYMO_CORE_PLUGIN_URL', plugin_dir_url(__FILE__));
        }

        if (!defined('GIFYMO_CORE_PLUGIN_FILE')) {
            define('GIFYMO_CORE_PLUGIN_FILE', __FILE__);
        }
    }
    ...
    private function load_textdomain() {
        $lang_dir      = dirname(plugin_basename(GIFYMO_CORE_PLUGIN_FILE)) . '/languages/';
        $lang_dir      = apply_filters('osf_languages_directory', $lang_dir);
        $locale        = apply_filters('plugin_locale', get_locale(), 'gifymo-core');
        $mofile        = sprintf('%1$s-%2$s.mo', 'gifymo-core', $locale);
        $mofile_local  = $lang_dir . $mofile;
        $mofile_global = WP_LANG_DIR . '/gifymo-core/' . $mofile;

        if (file_exists($mofile_global)) {
            load_textdomain('gifymo-core', $mofile_global);
        } else {
            if (file_exists($mofile_local)) {
                load_textdomain('gifymo-core', $mofile_local);
            } else {
                load_plugin_textdomain('gifymo-core', false, $lang_dir);
            }
        }
    }
    ...
    public function plugin_update() {
        ...
        Puc_v4_Factory::buildUpdateChecker(
            'http://source.wpopal.com/gifymo/dummy_data/update-plugin.json',
            __FILE__,
            'gifymo-core'
        );
    }
...
//		require_once 'inc/vendors/cmb2/custom-fields/text_price.php';
    ...
    public static function getInstance() {
        if (!isset(self::$instance) && !(self::$instance instanceof GifymoCore)) {
            self::$instance = new GifymoCore();
        }
