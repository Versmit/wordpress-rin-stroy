...
class OSF_Theme_Setup {
    ...
    public function __construct() {
        ...
        //
        //add_action('gifymo_single_entry_footer', array($this, 'render_related_posts'));
...
//		add_action( 'bcn_after_fill', [ $this, 'breadcrumb_remove_current_item' ] );
    ...
    public function widgets_init() {
        register_sidebar(array(
            'name'          => esc_html__('Single Posts Sidebar', 'gifymo-core'),
            'id'            => 'sidebar-single-post',
            'description'   => esc_html__('Add widgets here to appear in your sidebar on single posts.', 'gifymo-core'),
        ...
        register_sidebar(array(
            'name'          => esc_html__('Blog Sidebar', 'gifymo-core'),
            'id'            => 'sidebar-blog',
            'description'   => esc_html__('Add widgets here to appear in your sidebar on blog posts and archive pages.', 'gifymo-core'),
            'before_widget' => '<section id="%1$s" class="widget %2$s">',
            'after_widget'  => '</section>',
            'before_title'  => '<h2 class="widget-title">',
            'after_title'   => '</h2>',
        ...
        register_sidebar(array(
            'name'          => esc_html__('Page Sidebar', 'gifymo-core'),
            'id'            => 'sidebar-page',
            'description'   => esc_html__('Add widgets here to appear in your pages.', 'gifymo-core'),
            'before_widget' => '<section id="%1$s" class="widget %2$s">',
            'after_widget'  => '</section>',
            'before_title'  => '<h2 class="widget-title">',
            'after_title'   => '</h2>',
    ...
    public function modal_login_register() {
        include_once trailingslashit(GIFYMO_CORE_PLUGIN_DIR) . 'templates/modal/login-register-form.php';
    }
    ...
    private function set_main_nav_content($items) {
        ...
        $items .= '<a data-search-toggle="toggle" data-target=".' . $id . '" class="fa fa-search" title="' . __("search", "gifymo-core") . '"></a>';
    ...
    public function render_html_back_to_top() {
        if (get_theme_mod('osf_back_to_top_footer') == true) {
            echo sprintf('<a href="#" class="scrollup"><span class="scrollup-icon opal-icon-arrow-up"></span><span class="scrollup-label">%s</span></a>',
                esc_html__('Наверх', 'gifymo-core'));
        }
    }

    public function render_related_posts() {
        if (is_active_sidebar('sidebar-single-post') || (!class_exists('GifymoCore') && is_active_sidebar('sidebar-blog'))) {
            gifymo_fnc_related_post(2);
        } else {
            gifymo_fnc_related_post(2);
        }
    }
