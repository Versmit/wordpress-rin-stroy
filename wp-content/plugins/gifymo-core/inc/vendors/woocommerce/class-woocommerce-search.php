    ...
    class osf_WooCommerce_Search {
        ...
        public function ajax_search_product() {
            ...
            if ($the_query->have_posts()) :
                ...
                echo '</ul>';
                ?>
                <a class="button-link-search" href="<?php echo get_site_url() . '?s=' . esc_attr($_REQUEST['keyword']) . '&post_type=product'; ?>"><?php printf(__('Show All %s Results', 'gifymo-core'), $the_query->found_posts); ?>
                    <i class="opal-icon-arrow" aria-hidden="true"></i></a>
            <?php
            else:
                ?>
                <h4><?php echo __('No products found', 'gifymo-core'); ?></h4>
                <?php
                wp_reset_postdata();
            endif;
