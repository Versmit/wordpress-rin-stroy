...
if (!function_exists('osf_woocommerce_widget_shopping_cart_button_view_cart')) {
    ...
    function osf_woocommerce_widget_shopping_cart_button_view_cart() {
        echo '<a href="' . esc_url(wc_get_cart_url()) . '" class="button wc-forward"><span>' . esc_html__('View cart', 'gifymo-core') . '</span></a>';
    }
}
...
if (!function_exists('osf_woocommerce_widget_shopping_cart_proceed_to_checkout')) {
    ...
    function osf_woocommerce_widget_shopping_cart_proceed_to_checkout() {
        echo '<a href="' . esc_url(wc_get_checkout_url()) . '" class="button checkout wc-forward"><span>' . esc_html__('Checkout', 'gifymo-core') . '</span></a>';
    }
}
...
if (!function_exists('osf_cart_link')) {
    ...
    function osf_cart_link() {
        if (!empty(WC()->cart) && WC()->cart instanceof WC_Cart) {
            $items = '';
            $items .= '<a data-toggle="toggle" class="cart-contents header-button" href="' . esc_url(wc_get_cart_url()) . '" title="' . __("View your shopping cart", "gifymo-core") . '">';
...
if (!function_exists('osf_cart_count_text')) {
    ...
    function osf_cart_count_text() {
        if (!empty(WC()->cart) && WC()->cart instanceof WC_Cart) {
            return '<span class="count-text">' . wp_kses_data(_n("item", "items", WC()->cart->get_cart_contents_count(), "gifymo-core")) . '</span>';
        }
...
if (!function_exists('osf_product_columns_wrapper')) {
    ...
    function osf_product_columns_wrapper() {
        ...
        if (get_theme_mod('osf_woocommerce_product_style', 'grid') == 'list') {
            $columns = 1;
        }
...
if (!function_exists('osf_handheld_footer_bar_search')) {
    ...
    function osf_handheld_footer_bar_search() {
        echo '<a href="">' . esc_attr__('Search', 'gifymo-core') . '</a>';
        osf_product_search();
    }
}
...
if (!function_exists('osf_handheld_footer_bar_cart_link')) {
    ...
    function osf_handheld_footer_bar_cart_link() {
        ...
        <a class="footer-cart-contents" href="<?php echo esc_url(wc_get_cart_url()); ?>"
           title="<?php esc_attr_e('View your shopping cart', 'gifymo-core'); ?>">
...
if (!function_exists('osf_handheld_footer_bar_account_link')) {
    ...
    function osf_handheld_footer_bar_account_link() {
        echo '<a href="' . esc_url(get_permalink(get_option('woocommerce_myaccount_page_id'))) . '">' . esc_attr__('My Account', 'gifymo-core') . '</a>';
    }
}
    ...
    function osf_woocommerce_product_best_selling() {
        ...
        <div class="best-selling">
            <div class="best-selling-inner">
                <h4 class="best-selling-title"><?php echo esc_html__('Trending', 'gifymo-core'); ?></h4>
                <ul class="product_list_widget product-best-selling">
                    ...
                    if ($loop->have_posts()) {
                        ...
                        echo __('No products found', 'gifymo-core');
                    }
                    wp_reset_postdata();
                    ?>
                </ul>
...
if (!function_exists('osf_woocommerce_get_product_label_stock')) {
    function osf_woocommerce_get_product_label_stock() {
        ...
        if ($product->get_stock_status() == 'outofstock') {
            echo '<span class="stock-label outofstock"><span>' . esc_html__('Out Of Stock', 'gifymo-core') . '</span></span>';
        } elseif ($product->get_stock_status() == 'instock') {
            echo '<span class="stock-label instock"><span>' . esc_html__('In Stock', 'gifymo-core') . '</span></span>';
        } else {
            echo '<span class="stock-label onbackorder"><span>' . esc_html__('On backorder', 'gifymo-core') . '</span></span>';
        }
    }
}
...
if (!function_exists('osf_woocommerce_get_product_label_new')) {
    function osf_woocommerce_get_product_label_new() {
        ...
        if ((time() - (60 * 60 * 24 * $newness_days)) < $created) {
            echo '<span class="new-label"><span>' . esc_html__('New!', 'gifymo-core') . '</span></span>';
        }
    }
}
...
if (!function_exists('osf_woocommerce_get_product_label_sale_number')) {
    function osf_woocommerce_get_product_label_sale_number() {
        ...
        if ($product->is_on_sale() && $product->is_type('simple')) {
            ...
            echo '<span class="onsale">-' . esc_html($ratio) . '% ' . esc_html__('Off', 'gifymo-core') . ' </span>';
        }
    }
}
...
if (!function_exists('osf_woocommerce_get_product_label_sale')) {
    function osf_woocommerce_get_product_label_sale() {
        ...
        global $product;
        if ($product->is_on_sale() && $product->is_type('simple')) {
            echo '<span class="onsale">' . esc_html__('Sale', 'gifymo-core') . ' </span>';
        }
    }
}
...
if (!function_exists('osf_woocommerce_get_product_label_feature')) {
    function osf_woocommerce_get_product_label_feature() {
        ...
        if ($product->is_featured()) {
            echo '<span class="trend"><span>' . esc_html__('Trend', 'gifymo-core') . '</span></span>';
        }
    }
}
...
if (!function_exists('osf_woocommerce_set_register_text')) {
    function osf_woocommerce_set_register_text() {
        echo '<div class="user-text">' . __("Creating an account is quick and easy, and will allow you to move through our checkout quicker.", "gifymo-core") . '</div>';
    }
}
...
if (!function_exists('osf_woocommerce_time_sale')) {
    function osf_woocommerce_time_sale() {
        ...
        if ($time_sale) {
            ...
            echo '<div class="time">
                    <div class="deal-text d-none">' . esc_html__('Hurry up. Offer end in', 'gifymo-core') . '</div>
                    <div class="opal-countdown clearfix"
                        data-countdown="countdown"
                        data-days="' . esc_html__("days", "gifymo-core") . '" 
                        data-hours="' . esc_html__("hours", "gifymo-core") . '"
                        data-minutes="' . esc_html__("mins", "gifymo-core") . '"
                        data-seconds="' . esc_html__("secs", "gifymo-core") . '"
                        data-Message="' . esc_html__('Expired', 'gifymo-core') . '"
...
if (!function_exists('osf_active_filters')) {
    function osf_active_filters() {
        if (is_filtered()) {
            ...
            echo '<div class="osf-active-filters"> <span class="osf_active_filters_label">' . esc_html__('Active Filters: ', 'gifymo-core') . '</span>';
            the_widget('WC_Widget_Layered_Nav_Filters');
            echo '
                <a class="clear-all" href="' . esc_url($link_remove_all) . '">' . __('Clear Filters', 'gifymo-core') . '</a>
            </div>';
        }
...
if (!function_exists('osf_single_product_social')) {
    function osf_single_product_social() {
        if (get_theme_mod('osf_socials')) {
            $template      = GIFYMO_CORE_PLUGIN_DIR . 'templates/socials.php';
            $socials_label = true;
...
if (!function_exists('osf_woocommerce_deal_progress')) {
    function osf_woocommerce_deal_progress() {
        ...
        <div class="deal-sold">
            <span class="deal-text d-block"><span><?php esc_html_e('Hurry! only', 'gifymo-core') ?></span>
                <span class="c-primary"><?php echo esc_attr(trim($limit - $sold)) ?></span> <span><?php esc_html_e('left in stock.', 'gifymo-core') ?></span></span>
            <div>
...
function otf_woocommerce_recently_viewed_product() {
    if (get_theme_mod('otf_woocommerce_extra_enable_product_recently_viewed', false)) {
        ...
        if (!empty($_COOKIE['otf_woocommerce_recently_viewed'])) {
            ...
            echo '<h2 class="otf-woocommerce-recently-viewed">' . esc_html__('Your Recently Viewed', 'gifymo-core') . '</h2>';
...
function osf_woocommerce_pagination_args($args) {
    $args['prev_text'] = '<span class="opal-icon-angle-left"></span>' . __('Пред.', 'gifymo-core');
    $args['next_text'] = __('След.', 'gifymo-core') . '<span class="opal-icon-angle-right"></span>';
    $args['type']      = 'plain';
...
function filter_woocommerce_empty_price_html($var, $instance) {
    return esc_html__('Free', 'gifymo-core');
}
...
function woocommerce_custom_get_availability_text($availability, $product) {
    $availability = '<span class="label">' . esc_html__('Availability: ', 'gifymo-core') . '</span><span class="stock-availability">' . $availability . '</span>';
...
if (!function_exists('osf_button_grid_list_layout')) {
    function osf_button_grid_list_layout() {
        ...
        <div class="gridlist-toggle desktop-hide-down">
            <a href="<?php echo esc_url(add_query_arg('osf_woocommerce_product_style', 'grid')); ?>" id="grid" class="<?php echo isset($_GET['osf_woocommerce_product_style']) && $_GET['osf_woocommerce_product_style'] == 'list' ? '' : 'active'; ?>" title="<?php echo esc_attr__('Grid View', 'gifymo-core'); ?>"><i class="opal-icon-th-grid" aria-hidden="true"></i></a>
            <a href="<?php echo esc_url(add_query_arg('osf_woocommerce_product_style', 'list')); ?>" id="list" class="<?php echo isset($_GET['osf_woocommerce_product_style']) && $_GET['osf_woocommerce_product_style'] == 'list' ? 'active' : ''; ?>" title="<?php echo esc_attr__('List View', 'gifymo-core'); ?>"><i class="opal-icon-th-list" aria-hidden="true"></i></a>
        </div>
        <?php
    }
}
...
function osf_woocommerce_show_product_sale_flash() {
    ...
    if ($product->is_on_sale()) :
        ...
        return '<span class="onsale"><span>' . esc_html__('Sale!', 'gifymo-core') . '</span></span>';
    endif;
