...
//                add_action('woocommerce_after_shop_loop', 'woocommerce_result_count', 5);
        ...
        public function mobile_handheld_footer_bar() {
            $links = array(
                'my-account' => '<a class="my-accrount-footer" href="' . esc_url(get_permalink(get_option('woocommerce_myaccount_page_id'))) . '">' . esc_attr__('My Account', 'gifymo-core') . '</a>',
                'search'     => '<a class="search-footer" href="">' . esc_attr__('Search', 'gifymo-core') . '</a><div class="site-search">' . get_search_form(false) . '</div>',
                'cart'       => ' <a class="footer-cart-contents" href="' . esc_url(wc_get_cart_url()) . '" 
                title="' . esc_attr__('View your shopping cart', 'gifymo-core') . '">  
 <span class="count">' . wp_kses_data(WC()->cart->get_cart_contents_count()) . '</span>
        ...
        public function label_tooltip() {
            ...
            echo '<div id="osf-woocommerce-cart">' . esc_html__('Add to cart', 'gifymo-core') . '</div>';
            echo '</div>';
        }
        ...
        public function added_to_cart_template() {
            $text = esc_html__('has been added to your cart', 'gifymo-core');
            echo <<<HTML
...
HTML;
        ...
        public function widgets_init() {
            register_sidebar(array(
                'name'          => __('WooCommerce Shop', 'gifymo-core'),
                'id'            => 'sidebar-woocommerce-shop',
                'description'   => __('Add widgets here to appear in your sidebar on shop page and archive shop pages.', 'gifymo-core'),
            ...
            register_sidebar(array(
                'name'          => __('WooCommerce Detail', 'gifymo-core'),
                'id'            => 'sidebar-woocommerce-detail',
                'description'   => __('Add widgets here to appear in your sidebar on single product page', 'gifymo-core'),
                'before_widget' => '<section id="%1$s" class="widget %2$s">',
                'after_widget'  => '</section>',
                'before_title'  => '<h2 class="widget-title">',
                'after_title'   => '</h2>',
        ...
        public function render_woocommerce_shop_canvas() {
            ...
            if (is_active_sidebar('sidebar-woocommerce-shop') && osf_is_product_archive() && $position != '1c') {
                echo '<div id="opal-canvas-filter" class="opal-canvas-filter position-' . $position . '"><span class="filter-close">' . esc_html__('CLOSE', 'gifymo-core') . '</span><div class="opal-canvas-filter-wrap">';
        ...
        public function render_button_shop_canvas() {
            $position = get_theme_mod('osf_woocommerce_archive_layout', '2cl');
            if (is_active_sidebar('sidebar-woocommerce-shop') && osf_is_product_archive() && $position != '1c') {
                echo '<button class="filter-toggle" aria-expanded="false"><span class="filter-icon"></span>' . esc_html__('Filter', 'gifymo-core') . '</button>';
            }
        }
            ...
            if (!empty($query)) {
                ...
                if (is_object($product_object)) {
                    ...
                    if (!empty($product_sku)) {
                        $product_sku_display = ' - ' . __('Sku', 'gifymo-core') . ': ' . $product_sku;
                    }

                    $product_title_display = '';
                    if (!empty($product_title)) {
                        $product_title_display = ' - ' . __('Title', 'gifymo-core') . ': ' . $product_title;
                    }

                    $product_id_display = __('Id', 'gifymo-core') . ': ' . $product_id;
        ...
        public function productCategoryAutocompleteRender($query) {
            ...
            $term_slug_display = '';
            if (!empty($term_slug)) {
                $term_slug_display = ' - ' . __('Sku', 'gifymo-core') . ': ' . $term_slug;
            }

            $term_title_display = '';
            if (!empty($term_title)) {
                $term_title_display = ' - ' . __('Title', 'gifymo-core') . ': ' . $term_title;
            }

            $term_id_display = __('Id', 'gifymo-core') . ': ' . $term_id;
        ...
        public function taxonomy_metaboxes() {
            ...
            $cmb_term = new_cmb2_box(array(
                'id'           => 'product_cat',
                'title'        => __('Product Metabox', 'gifymo-core'), // Doesn't output for term boxes
        ...
        public function product_video_custom_field() {
            ...
            $cmb->add_field(array(
                'name' => __('Product video', 'gifymo-core'),
                'desc' => __('Supports video from YouTube, Vimeo and Rutube.', 'gifymo-core'),
                'id'   => $prefix . 'video',
                'type' => 'oembed',
            ));
        ...
        public function woocommerce_scripts() {
            ...
            wp_enqueue_script('osf-woocommerce-main', trailingslashit(GIFYMO_CORE_PLUGIN_URL) . 'assets/js/woocommerce/main.js', array(
                'jquery',
                'wp-util'
            ), false, true);
            wp_localize_script('osf-woocommerce-main', 'gifymo', array(
                'ajaxurl' => admin_url('admin-ajax.php'),
            ));

            if (is_product()) {
                wp_enqueue_script('osf-woocommerce-single', trailingslashit(GIFYMO_CORE_PLUGIN_URL) . 'assets/js/woocommerce/single.js', array(), false, true);
                if (get_theme_mod('osf_woocommerce_single_product_tab_style', 'tab') == 'accordion') {
                    wp_enqueue_script('osf-woocommerce-single-accordion', trailingslashit(GIFYMO_CORE_PLUGIN_URL) . 'assets/js/woocommerce/woocommerce-accordion.js', array(), false, true);
                }
            }
        ...
        public function custom_comment_form($comment_form) {
            ...
            $comment_form['fields']        = array(
                'author' => '<p class="comment-form-author"><input id="author" name="author" type="text" placeholder="' . esc_attr__("Name", "gifymo-core") . '" value="' . esc_attr($commenter['comment_author']) . '" size="30" required /></p>',
                'email'  => '<p class="comment-form-email"><input id="email" name="email" type="email" placeholder="' . esc_attr__("Email", "gifymo-core") . '" value="' . esc_attr($commenter['comment_author_email']) . '" size="30" required /></p>',
                //                'url'    => '<p class="comment-form-url"> <input id="url" name="url" type="url" placeholder="' . esc_attr__("Website", "gifymo-core") . '" value="' . esc_attr($commenter['comment_author_url']) . '" size="30" maxlength="200" /></p>'
            );
            $comment_form['comment_field'] = '';

            if (wc_review_ratings_enabled()) {
                $comment_form['comment_field'] = '<div class="comment-form-rating"><label for="rating">' . esc_html__('Ваш отзыв', 'gifymo-core') . '</label><select name="rating" id="rating" required>
						<option value="">' . esc_html__('Rate&hellip;', 'gifymo-core') . '</option>
						<option value="5">' . esc_html__('Perfect', 'gifymo-core') . '</option>
						<option value="4">' . esc_html__('Good', 'gifymo-core') . '</option>
						<option value="3">' . esc_html__('Average', 'gifymo-core') . '</option>
						<option value="2">' . esc_html__('Not that bad', 'gifymo-core') . '</option>
						<option value="1">' . esc_html__('Very poor', 'gifymo-core') . '</option>
					</select></div>';
            }


            $comment_form['comment_field'] .= '<p class="comment-form-comment"><textarea id="comment" name="comment" cols="45" rows="6" required placeholder="' . esc_attr__("Текст отзыва", "gifymo-core") . '"></textarea></p>';
            $comment_form['submit_button'] = '<button name="%1$s" type="submit" id="%2$s" class="%3$s"><span>%4$s</span></button>';
            $comment_form['label_submit']  = __('Оставить отзыв', 'gifymo-core');
