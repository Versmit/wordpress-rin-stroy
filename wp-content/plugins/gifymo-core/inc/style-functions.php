...
function osf_theme_custom_css() {
    $a1root      = include trailingslashit(GIFYMO_CORE_PLUGIN_DIR) . 'inc/customize/a1root.php';
    $grid        = include trailingslashit(GIFYMO_CORE_PLUGIN_DIR) . 'inc/customize/grid.php';
    $button      = include trailingslashit(GIFYMO_CORE_PLUGIN_DIR) . 'inc/customize/button.php';
    $error_404   = include trailingslashit(GIFYMO_CORE_PLUGIN_DIR) . 'inc/customize/error-404.php';
    $heading     = include trailingslashit(GIFYMO_CORE_PLUGIN_DIR) . 'inc/customize/heading.php';
    $main_layout = include trailingslashit(GIFYMO_CORE_PLUGIN_DIR) . 'inc/customize/main-layout.php';
    $page_bg     = include trailingslashit(GIFYMO_CORE_PLUGIN_DIR) . 'inc/customize/page-bg.php';
    $page_title  = include trailingslashit(GIFYMO_CORE_PLUGIN_DIR) . 'inc/customize/page-title.php';
    $css         = <<<CSS
{$a1root}
{$grid}
{$error_404}
...
// Footer Css
    ...
    if (is_page()) {
        ...
        $page_breadcrumb_alignment = get_post_meta(get_the_ID(), 'osf_breadcrumb_alignment', 1);
        $page_breadcrumb_padding   = get_post_meta(get_the_ID(), 'osf_breadcrumb_padding', 1);

//        page background color
        $page_background_color = get_post_meta(get_the_ID(), 'osf_page_background_color', 1);

        if (!empty($page_breadcrumb_alignment)) {
            $alignmen = $page_breadcrumb_alignment;
            if ($page_breadcrumb_alignment == 'left') {
                $alignmen = 'flex-start';
            } elseif ($page_breadcrumb_alignment == 'right') {
                $alignmen = 'flex-end';
            }
            $css .= '.page-title-bar .page-title-bar-inner {text-align: ' . $page_breadcrumb_alignment . ';}';
            $css .= '.page-title-bar .page-title-bar-inner .breadcrumb {justify-content: ' . $alignmen . ';}';
        }

        if (!empty($page_breadcrumb_padding)) {
            $css .= '.page-title-bar .page-title-bar-inner {padding: ' . $page_breadcrumb_padding . 'px 0;}';
        }
        ...
        if ($page_title_color && $page_title_color != '#666') {
            ...
            $css                  .= '.page-title-bar .page-title{' . $page_title_css_color . '}';
        }
