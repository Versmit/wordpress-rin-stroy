...
function osf_get_google_fonts() {
    $content = file_get_contents(GIFYMO_CORE_PLUGIN_DIR . 'webfonts.json');
...
if (!function_exists('osf_scrape_instagram')) {
    ...
    function osf_scrape_instagram($username, $slice = 9) {
        ...
        if (false === ($instagram = get_transient('otf-instagram-media-new-' . sanitize_title_with_dashes($username)))) {
            ...
            if (!$insta_array) {
                return new WP_Error('bad_json', esc_html__('Instagram has returned invalid data.', 'gifymo-core'));
            }
            ...
            if (isset($insta_array['entry_data']['UserProfile'][0]['userMedia'])) {
                ...
                return new WP_Error('bad_json_2', esc_html__('Instagram has returned invalid data.', 'gifymo-core'));
            }
            ...
            switch ($type) {
                ...
                case 'old_2':
                    foreach ($images as $image) {
                        ...
                        $caption = esc_html__('Instagram Image', 'gifymo-core');
...
function time_elapsed_string($datetime, $full = false) {
    ...
    $string = array(
        'y' => __('year', 'gifymo-core'),
        'm' => __('month', 'gifymo-core'),
        'w' => __('week', 'gifymo-core'),
        'd' => __('day', 'gifymo-core'),
        //        'h' => __('hour', 'gifymo-core'),
        //        'i' => __('minute', 'gifymo-core'),
        //        's' => __('second', 'gifymo-core'),
    );
    ...
    return $string ? implode(', ', $string) . __(' left', 'gifymo-core') : __('just now', 'gifymo-core');
}
...
if ( ! function_exists( 'osf_vertical_navigation' ) ) {
    ...
    function osf_vertical_navigation() {
        ...
        if ( isset( get_nav_menu_locations()['vertical'] ) ) {
            ...
            $menu_name      = ( isset( $menu_object->name ) ? $menu_object->name : esc_html__( 'Vertical Menu', 'gifymo-core' ) );

            ?>
            <nav class="vertical-navigation" aria-label="<?php esc_html_e( 'Vertical Navigation', 'gifymo-core' ); ?>">
